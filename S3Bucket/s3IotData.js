...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await IotData.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'iot_data/iotData.json';
        ...
        const deleteResult = await IotData.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
