...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await payment.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'payment_data/paymentData.json';
        ...
        const deleteResult = await payment.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
