...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await TotalConsumption.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'totalConsumption_data/totalConsumptionData.json';
        ...
        const deleteResult = await TotalConsumption.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
module.exports = { uploadDataToS3AndClearDB, setupCronJobS3TotalConsumptionData };
