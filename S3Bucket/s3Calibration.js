...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await calibration.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'calibration_data/calibrationData.json';
        ...
        const deleteResult = await calibration.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
const setupCronJobS3Calibration = () => {
    cron.schedule('15 */1 * * *', () => { // Runs at the 15th minute of every hour
        ...
        console.log(`Running Calibration Data upload and cleanup at IST: ${currentTimeIST}`);
...
module.exports = { uploadDataToS3AndClearDB, setupCronJobS3Calibration };
