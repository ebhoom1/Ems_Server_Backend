...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await chat.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'chat_data/chatData.json';
        ...
        const deleteResult = await chat.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
const setupCronJobS3Chat = () => {
    cron.schedule('15 */1 * * *', () => { // Runs at the 15th minute of every hour
        ...
        console.log(`Running Chat Data upload and cleanup at IST: ${currentTimeIST}`);
...
module.exports = { uploadDataToS3AndClearDB, setupCronJobS3Chat };
