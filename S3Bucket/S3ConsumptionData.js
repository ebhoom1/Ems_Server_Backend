...
const consumptionData = require('../models/Consumption');
...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await consumptionData.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const fileName = 'consumption_data/consumptionData.json';
        ...
        const deleteResult = await consumptionData.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
const setupCronJobS3ConsumptionData = () => {
    cron.schedule('15 */1 * * *', () => { // Runs at the 15th minute of every hour
        const currentTimeIST = moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:mm:ss');
        console.log(`Running Consumption Data upload and cleanup at IST: ${currentTimeIST}`);
...
module.exports = { uploadDataToS3AndClearDB, setupCronJobS3ConsumptionData };
