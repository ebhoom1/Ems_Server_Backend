...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await AverageData.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const deleteResult = await AverageData.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
const setupCronJobS3Average = () => {
    cron.schedule('15 * * * *', () => {
        ...
        console.log(`Running hourly data average upload and cleanup at IST: ${currentTimeIST}`);
