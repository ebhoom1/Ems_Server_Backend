...
const uploadDataToS3AndClearDB = async () => {
    try {
        ...
        const data = await hourlyData.find({ timestamp: { $gte: oneHourAgo, $lte: now } }).lean();
        ...
        const deleteResult = await hourlyData.deleteMany({ timestamp: { $gte: oneHourAgo, $lte: now } });
...
const setupCronJobS3HourlyData = () => {
    cron.schedule('15 */1 * * *', () => { // Runs at the 15th minute of every hour
        ...
        console.log(`Running Hourly Data upload and cleanup at IST: ${currentTimeIST}`);
...
module.exports = { uploadDataToS3AndClearDB, setupCronJobS3HourlyData };
