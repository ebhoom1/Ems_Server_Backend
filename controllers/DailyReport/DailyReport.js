...
const generatePDFForUser = async (companyName, userName, stackNames, industryType) => {
    ...
    try {
        ...
        const consumptionDataDoc = await ConsumptionData.findOne({ userName });
        const differenceDataDoc = await DifferenceData.find({ userName });
        ...
        for (const stackName of stackNames) {
            const iotDataDoc = await IotData.findOne({ userName, 'stackData.stackName': stackName.name });
            const minMaxDataDoc = await MaxMinData.findOne({ userName, stackName: stackName.name });
            const calibrationDataDoc = await calibrationExceedValues.findOne({ industryType });
